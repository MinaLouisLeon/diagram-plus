 ...
 *
 * It is a typed tree rather than markup or a picture, for the same reason the
 * diagram is typed blocks rather than prose. A `button` here is a button —
 * something with a label, a variant, and an action that names an endpoint in
 * the diagram — so the editor can draw it, a person can drag it, Claude can
 * write it, and the implementer can read a contract out of it. Markup would
 * render beautifully and round-trip badly; a picture would round-trip not at
 * all.
 *
 * Nothing in here is a stylesheet. Elements point at *tokens* — `heading.lg`,
 * `accent`, `radius.md` — defined once in the design system, so a screen says
 * "this is a large heading in the accent colour" and the project decides what
 * that means. That is what keeps thirty screens looking like one product, and
 * what lets a stack change without redrawing anything.
 *
...
export function migrateDesign(raw: unknown): unknown {
  ...
  if (version < 1) doc['formatVersion'] = DESIGN_FORMAT_VERSION;
  return doc;
}
